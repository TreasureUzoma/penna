...
/**
 * Account-wide counterpart to the per-project ActivityFeed
 * (projects/[id]/components/overview/activity-feed.tsx) — same visual
 * language, but spans every project the user belongs to, so each item
 * links out to its own project.
 */
        ...
        <div className="space-y-6">
          {activity?.length === 0 && (
            <p className="text-sm text-muted-foreground py-4 text-center">
              No posts sent yet across your projects.
            </p>
          )}
          {activity?.map((item) => (
            <Link
              key={item.id}
              href={`/projects/${item.projectSlug}/posts/${item.id}`}
