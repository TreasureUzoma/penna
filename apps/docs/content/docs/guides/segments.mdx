...
When adding a subscriber manually:
1. Enter their email
2. Check the segments you want to add them to
3. Click "Create"
...
3. Select/deselect segments
4. Save changes

**Bulk addition:**
1. Select multiple subscribers from list
...
```javascript
await fetch('https://api.penna.dev/api/v1/external/newsletters/subscriber/new', {
  method: 'POST',
  headers: {
  ...
  body: JSON.stringify({
    ...
    segmentIds: ['segment-id-1', 'segment-id-2']
  })
});
```
...
2. In the recipients section, select:
   - **All subscribers**, or
   - **Specific segments** (choose one or more)
3. Review the total recipient count
4. Send or schedule

**Example:**
```
To: Web Development + Design Resources segments
Recipients: 2,345 subscribers
```
...
Select multiple segments to send to combined audience:
...
Penna automatically deduplicates - subscribers in multiple segments only receive one email.
...
- Growth over time (coming soon)
- Recent additions
- Engagement metrics (coming soon)

### Editing Segments

To rename or update description:
1. Open segment
2. Click "Edit"
...
> [!NOTE]
> Deleting a segment does NOT delete the subscribers - they remain in your database and other segments.
...
- [Improve engagement](/docs/best-practices/subscriber-engagement)
- [Track analytics](/docs/guides/analytics)

---

> [!TIP]
> Effective segmentation is about understanding your audience and delivering relevant content. Start simple, measure results, and refine over time.
