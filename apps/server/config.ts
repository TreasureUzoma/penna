...
const envSchema = z
  .object({
    ...
    // How long a moderation request may take before we fail open and continue
    ...
    // Cloudflare R2 for image uploads in newsletter posts
  ...
  .superRefine((env, ctx) => {
    ...
    if (env.NODE_ENV === "production" && !env.SES_NOTIFICATIONS_TOPIC_ARN) {
      ctx.addIssue({
        ...
        message:
          "SES_NOTIFICATIONS_TOPIC_ARN is required in production for bounce and complaint suppression.",
      });
    }
  });
